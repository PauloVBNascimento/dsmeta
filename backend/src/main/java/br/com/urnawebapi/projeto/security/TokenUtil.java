...
public class TokenUtil {
    ...
    private static boolean expiracaoValida(Date expiration) {
        return expiration.after(new Date(System.currentTimeMillis()));
    }

    private static boolean emissorValido(String emissor) {
        return emissor.equals(EMISSOR);
    }

    private static boolean objetoValido(String username) {
        return username != null && username.length() > 0;
    }

    public static Authentication validar(HttpServletRequest request) {
        ...
        if (objetoValido(username) && emissorValido(issuer) && expiracaoValida(expira)) {
